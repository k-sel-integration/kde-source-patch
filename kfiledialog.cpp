...
/* This file is part of the KDE libraries
    Copyright (C) 1997, 1998 Richard Moore <rich@kde.org>
				  ...
				  2014 KDE-SELENIUM-INTEGRATION-TEAM <kde.selenium.integration.team@gmail.com>
...
QString correctoutput(QString command, bool& ok){
   ok = true;
   char* path = getenv("K_SEL_SUPPORT_FOLDER");
   char* ext  = getenv("K_SEL_SUPPORT_EXTENSION");
   if (path !=0 && ext != 0 ){
     command = QString(path) + command + QString(ext);
     QProcess proc; 
     proc.start(command);
...
int delay(bool& ok){
  return correctoutput("getdelay", ok).toInt(&ok);
}

QString url(bool& ok){
  return correctoutput("geturl", ok);
}
...
const bool NATIVE_FILEDIALOGS_BY_DEFAULT = false; // Selenium integration 2014 - was true;
#else
...
//Selenium 1ntegration 2014
void KFileDialog::selenSupport()
{
    bool ok;
...

{
  { //Selenium integration 2014
    bool ok; 
    int delay = seleniumsupport::delay(ok);
    if(ok)  QTimer::singleShot(delay, this, SLOT(selenSupport()));
  }
